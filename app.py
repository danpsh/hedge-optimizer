                        ...
                        for game in games:
                            ...
                            for s in source_odds:
                                ...
                                if promo_type == "Profit Boost (%)":
                                    ...
                                    h_needed = round((max_wager * (s_m + 1 - 0.70)) / (h_m + 1 + 0.70))
...
Actually, checking back against your request, you wanted to round to the nearest dollar for the hedge amount. While the Excel math gives you the exact decimal, rounding to the nearest dollar is often preferred by bettors to avoid "limit flags" from sportsbooks that look for suspicious decimal-cent wagers.

Here is the full code with the rounding logic applied to the hedge amount and the indentation issues resolved.
    ...
    with st.form("manual_calc_form"):
        ...
        if st.form_submit_button("Calculate Hedge", use_container_width=True):
            try:
                ...
                else: # No-Sweat Rounding Applied
                    mc = float(m_conv)/100 
