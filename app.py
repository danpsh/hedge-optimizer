...
with st.expander("Combine two different promos (Auto-scan)", expanded=True):
    with st.form("multi_promo_scan_form"):
        ...
        multi_selected_sports = []
        m_sport_cols = st.columns(len(sport_labels))
        for i, label in enumerate(sport_labels):
            with m_sport_cols[i]:
                if st.checkbox(label, value=all_clicked, key=f"multi_cb_{label}"):
                    multi_selected_sports.append(sports_map[label])

        run_multi_scan = st.form_submit_button("Run multi-promo scanner", use_container_width=True)

if run_multi_scan:
    api_key = st.secrets.get("ODDS_API_KEY", "")
    if not multi_selected_sports:
